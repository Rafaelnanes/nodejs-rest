module.exports = function (app) {
    ...
    var findById = function (id, callback) {
        ProductModel.findById(id, function (error, doc) {
            var response = utils.defaultDaoHandler(error, doc, 'Error getting product');
            callback(response);
        });
    }
    ...
    var findByQuery = function (query, callback) {
        ProductModel.find(query, 'name' /*properties returned*/, function (error, doc) {
            var response = utils.defaultDaoHandler(error, doc, 'Error getting products');
            callback(response);
        })
    }
    ...
    var save = function (product, callback) {
        ...
        productModel.save(function (error, doc) {
            var response = utils.defaultDaoHandler(error, doc, 'Error saving product', 'Product saved');
            callback(response);
        });
    }
